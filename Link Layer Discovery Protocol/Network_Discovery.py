...
# Some Minor changes for term paper of IEEE TSN Course
...
class SimpleSwitch13(app_manager.RyuApp):
    ...
    # Function to be processed by Thread, Getting the Information from Ryu controller
    # plotting the Network Graph after some packets transmission
    def myfunction(self):
        ...
        self.logger.info("NetLab#######Topology (GRAPH)#######Netlab")
        ...
        for item in range(len(self.links)):
            _list.append(tuple((self.links[item][0:2])))
        _list_hosts = []
        hosts = []
        alpha = 'a'
        ...
        for item in range(len(self.hosts)):
            _list_hosts.append(tuple((self.hosts[item][1], hosts[item])))
        _nodes = switches + hosts
        G.add_nodes_from(_nodes)
        _edges = _list + _list_hosts
